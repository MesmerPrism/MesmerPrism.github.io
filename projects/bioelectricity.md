...
## Observed, curated, and educational layers stay separated
 ...
 a neuron cell-cloud marker GLB, a nuclei centroid marker GLB, and an
 allometry FirstPoint marker GLB derived from the Zenodo ETLSM dataset,
 a parsed-but-deferred sidecar summary for
 Zenodo 12533272 , and two registered model-inspired replay
 manifests: the original Sketchfab schematic replay and a Zenodo
 neuron-cloud GIF replay. The observed Zenodo GLBs are not scientific
 simulation outputs. The 12533272 sidecar records a large tiled
 volume layout, not a browser mesh. A validation chain keeps
 ...
 a GIF generated from the reviewed neuron-cloud GLB positions, with the
 dynamics labeled as model-inspired rather than observed. It then separates
...
### Source-bounded atlas records
 ...
 Observed
 Zenodo posterior/anterior muscle SpatialGraph GLB line overlays, a neuron cell-cloud marker GLB, a nuclei centroid marker GLB, and an allometry FirstPoint marker GLB
 Curated
 ...
 Replay
 Two model-inspired replays: one Sketchfab schematic adapter and one Zenodo neuron-cloud GIF manifest
...
## Validated Planarian 3D loops and Zenodo point-cloud replay
...
## Matched to original sources, but not calibrated yet
 ...
 Result(2).Label-Analysis(2) FirstPoint anchor marker cloud. The
 same reviewed neuron-cloud GLB now has a public model-inspired dynamics GIF
 replay. It also has a Quest Browser validation lane that needs a refreshed headset
