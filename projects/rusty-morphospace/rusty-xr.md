...
## Use Rusty XR as the legacy collection.

 Rusty XR is still useful when older Quest examples, passthrough notes,
 companion-tool contracts, and source-taxonomy vocabulary need to be
 inspected. It should not be treated as the current umbrella for new
...
## Plain terms
 ...
 Legacy collection
 Public historical Quest/XR material retained for reference and compatibility.

 Passthrough
 A compositor-managed background route, not a raw camera texture the app can freely sample.

 Camera input
 App-visible camera frames when a route explicitly owns acquisition and evidence.

 MediaProjection
 Final-display capture evidence, separate from raw camera, passthrough, depth, or geometry truth.

 Companion tools
 Operator workflows for install, launch, casting, diagnostics, and evidence packaging.
