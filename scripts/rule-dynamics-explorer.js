(() => {
    ...
    function integrateStep(state, preset, t, dt) {
        ...
        const driveE = 10.5 * state.e - 9.3 * state.i - 2.2 + 2.8 * s;
        const driveI = 9.2 * state.e - 1.8 * state.i - 2.6 + 0.35 * s;
    ...
    function drawModes(canvas, preset) {
        ...
        const angles = preset.family === "stripe"
            ? [preset.angle]
            : [0, tau / 3, (2 * tau) / 3];
        ...
        angles.forEach((angle) => {
            ...
            ctx.strokeStyle = `rgb(${colors.accent.join(",")})`;
            ctx.lineWidth = preset.family === "stripe" ? 3 : 2.5;
            ctx.beginPath();
    ...
    class RuleExplorer {
        ...
        setPreset(key) {
            ...
            this.trace = buildTrace(this.preset);
            this.time = phase * this.trace.duration;
            this.presetButtons.forEach((button) => {
        ...
        updateReadouts(corrT, corr2T) {
            ...
            this.readouts.response.textContent = this.preset.response;
